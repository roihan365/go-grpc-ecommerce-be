...
var JwtEntityContextKeyValue JwtEntityContextKey = "jwt_entity"
...
func GetClaimsFromToken(token string) (*JwtClaims, error) {
	tokenClaim, err := jwt.ParseWithClaims(token, &JwtClaims{}, func(t *jwt.Token) (any, error) {
	...
	if claims, ok := tokenClaim.Claims.(*JwtClaims); ok {
		return claims, nil
	}
